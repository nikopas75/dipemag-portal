...
interface Props {
  ekpData: DimEkpData;
  setEkpData: React.Dispatch<React.SetStateAction<DimEkpData | null>>;
  mathData?: DimMathData | null;
}

export const DimEkpSection: React.FC<Props> = ({ ekpData, setEkpData, mathData }) => {
  ...
  const updateVal = (rowKey: string, specKey: string, val: string | number) => {
    ...
    const fieldName = `${rowKey}${specKey}` as keyof DimEkpData;
    const totalFieldName = `${rowKey}Total` as keyof DimEkpData;
    ...
    <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <span className="text-xs font-bold text-slate-800">
                Λειτουργικότητα Σχολικής Μονάδας:
              </span>
      ...
      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
        <h3 className="text-xs font-bold text-slate-800 mb-3 tracking-wider flex items-center space-x-2">
          ...
          <span>Κατανομή Ωρών Εκπαιδευτικού Προσωπικού (Δημοτικό)</span>
        </h3>
      ...
      {/* Remarks */}
      <div>
        <label className="block text-xs font-bold text-slate-800 mb-1">Παρατηρήσεις / Σημειώσεις Ωραρίου Εκπαιδευτικών</label>
        <textarea
          rows={3}
          value={ekpData.Parat || ''}
          onChange={e => setEkpData({ ...ekpData, Parat: e.target.value })}
          placeholder="Επιπλέον παρατηρήσεις για τις ώρες εκπαιδευτικών..."
